...
class FunctionRegistry:
    def __init__(self):
        ...
        self._required_functions = [
            "bricaud_cdom_model",
             "log_bricaud_cdom_model",
        ]

    def load_functions(self, backend: Backend):
        module = import_module_by_backend(backend, ModelName.CDOM)
...
def get_cdom_function(func_name: str, backend: Backend):
    return _function_registry.get_function(func_name, backend)

def create_model_function(func_name: str) -> Callable:
    def model_function(*args, **kwargs):
        backend = kwargs.pop("backend", Backend.NUMPY)
        return get_cdom_function(func_name, backend)(*args, **kwargs)
    return model_function

FUNCTION_REGISTRY_CDOM = {
    "bricaud": {
        "a": {
            "model": create_model_function("bricaud_cdom_model"),
        }
    },
    "log_bricaud": {
        "a": {
            "model": create_model_function("log_bricaud_cdom_model"),
        }
    },
}