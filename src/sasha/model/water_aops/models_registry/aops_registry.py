# File: shallow_sa/model/water_aops/aops_registry.py
...
def import_module_by_backend(backend: Backend, model_name: ModelName):
    try:
        module_path = f"....model.water_aops.backend.{backend.value}.{model_name.value}_{backend.value}"
...
class FunctionRegistry:
    ...
    def load_functions(self, backend: Backend):
        module = import_module_by_backend(backend, ModelName.AOPS)
...
FUNCTION_REGISTRY_AOPs = {
    "kc": {"model": create_model_function("compute_kb")},  # kc not different
    "kb": {"model": create_model_function("compute_kb")},
    "rrsdp": {"model": create_model_function("compute_rrsdp")},
}