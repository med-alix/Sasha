...
def log_likelihood(params, inv_var, observed_spectra, forward_model):
    ...
    mahalanobis_dist = inv_var * jnp.array([jnp.dot(row, row.T) for row in diff])
    log_likelihood_values = -0.5 * mahalanobis_dist
    return jnp.sum(log_likelihood_values)

log_likelihood = jax.jit(log_likelihood, static_argnums=(3,))
...
forward_model_jax = jax.jit(forward_model, static_argnums=(1,))


def jacob_dict_to_array(dict_data):
    arrays_list = [dict_data[key] for key in dict_data.keys()]
    final_array = jnp.stack(arrays_list, axis=-1)
    return final_array


# jacob_dict_to_array = jax.jit(jacob_dict_to_array, static_argnums=(0,))


def model_jacob_jax(param, forward_model):
    jacob = jacfwd(forward_model_jax)(param, forward_model)
    return jacob


model_jacob_jax = jax.jit(model_jacob_jax, static_argnums=(1,))


def model_jacob_numerical(param, forward_model):
    model_jac_dict = model_jacob_jax(param, forward_model)
    model_jac_arr = jacob_dict_to_array(model_jac_dict)
    if model_jac_arr.shape[0] > 1:
        return jnp.sum(model_jac_arr, axis=1)
    else:
        return model_jac_arr


# model_jacob_numerical = jax.jit(model_jacob_numerical, static_argnums=(1,))
...
def efim_numerical(param, inv_var, forward_model):
    ...
    model_jac_arr = jacob_dict_to_array(model_jac_dict)
    if model_jac_arr.shape[0] > 1:
        model_jac_arr = jnp.sum(model_jac_arr, axis=1)
        return ABA_product(model_jac_arr, inv_var)
    else:
        return ABA_product(model_jac_arr, inv_var)


def score_jax(param, inv_var, observed_spectra, forward_model):
    model_jac = model_jacob_numerical(param, forward_model)
    mean_spectra = forward_model(**param)
    diff = mean_spectra - observed_spectra
    score = inv_var * diff @ jnp.squeeze(model_jac)

    return score


score_jax = jax.jit(score_jax, static_argnums=(3,))
...
def score_numerical(param, inv_var, observed_spectra, forward_model):
    ...
    return jnp.array([score_dict[key] for key in score_dict.keys()])


score_numerical = jax.jit(score_numerical, static_argnums=(3,))


def hessian_jax(param, inv_var, observed_spectra, forward_model):
    return jacfwd(grad(log_likelihood))(param, inv_var, observed_spectra, forward_model)


hessian_jax = jax.jit(hessian_jax, static_argnums=(3,))
...
def flatten_hessian_results(hessian_results):
    ...
    for main_key in hessian_results.keys():
        ...
        for sub_key in hessian_results[main_key].keys():
            ...
            try:
                hess_mainkey_key = jnp.diagonal(hess)
            except ValueError:
                hess_mainkey_key = jnp.array([hess])
            flattened_results[main_key][sub_key] = hess_mainkey_key
    return flattened_results


# flatten_hessian_results = jax.jit(flatten_hessian_results, static_argnums=(0,))
...
def hessian_dict_to_array(hessian_dict):
    ...
    result_array = jnp.zeros((n, p, p), dtype=jnp.float32)
    # Prepare slices
    slices = [
    ...
    for i, j, hessian_slice in slices:
        if hessian_slice is not None:
            if hessian_slice.ndim == 0:
                hessian_slice = jnp.array([hessian_slice] * n)
            result_array = result_array.at[:, i, j].set(hessian_slice)
    return result_array


# Sample u
# hessian_dict_to_array = jax.jit(hessian_dict_to_array, static_argnums=(0,))


def hessian_numerical(param, inv_var, observed_spectra, forward_model):
    hess_jax = hessian_jax(param, inv_var, observed_spectra, forward_model)
    hess_flat = flatten_hessian_results(hess_jax)
    hess_arr = hessian_dict_to_array(hess_flat)
    return hess_arr


# hessian_numerical = jax.jit(hessian_numerical, static_argnums=(3,))
...
def score_analytical(observed_spectra, mean_spectra, model_jac, inv_var):
    ...
    dist = jnp.sum((observed_spectra - mean_spectra[:, :, None]), axis=-1)
    B_tiled = jnp.repeat(dist, p, axis=0)
    # Perform the matrix multiplication to get a (n * p, 1) shape result
    Score = inv_var * jnp.sum(A_reshaped.T * B_tiled, axis=1).reshape(-1, 1)
    return Score


@jit
def ABA_product(A, B):
    C_vectorized = B * jnp.matmul(A.transpose(0, 2, 1), A)
    return C_vectorized
...
def ABCBA_product(A, B, C):
    ...
    for i in range(n):
        A_block = A[i]
        result_block = jnp.dot(A_block.T, B * C[:, i])
...
def vectorized_outer_product(M, X):
    M_reshaped = M[:, :, jnp.newaxis]
    Y = M_reshaped * X[:, jnp.newaxis, :]
    return Y
...
def generalized_product_scalar(A1, A2, B, C):
    ...
    temp2 = jnp.matmul(temp1, C)  # Shape: (n, p, m)
    temp3 = B * temp2  # Shape: (n, p, m)
    result = jnp.matmul(temp3, A2)  # Shape: (n, p, p)
    return result
